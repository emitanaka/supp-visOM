...
(ref:mimicary) This figure recreates Figure 3 in @basole2021 using the `ggplot2` R-package [@ggplot2].
...
g1 <- df_miss %>% 
  ...
  scale_color_gradient2(low = "#3F6E9A", high = "#AB4C30", midpoint = median(df_miss$readmission_rate * 100), mid = "#FFFF00") +
  theme_void() +
...
g2 <- g1 %+% mutate(df_miss, y = colorectal_cancer_screenings) + 
  scale_color_gradient2(low = "#3F6E9A", high = "#AB4C30", midpoint = median(df_miss$colorectal_cancer_screenings), mid = "#FFFF00") +
